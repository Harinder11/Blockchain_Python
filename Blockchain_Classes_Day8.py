        ...
        try:
            with open('blockchain.txt', mode = 'r') as f: #mode='rb' for pickle
                ...
                for block in blockchain:
                    converted_tx = [Transaction(tx['sender'], tx['recipient'],tx['amount']) for tx in block['transaction']] #access block with [] as it is reading from the file where it is in the form of dictionary
                ...
                for tx in open_transaction:
                    updated_openblock = Transaction(tx['sender'],tx['recipient'],tx['amount'])
                    updated_openblockchain.append(updated_openblock)
                self.__open_transaction = updated_openblockchain   #open_transaction is also a list of transaction objects
